...
function App() {
  ...
  const isCypriotIssue = (story) => {
    const text = (story.headline + " " + story.summary + " " + (story.shortAnalysis || "")).toLowerCase();
    const keywords = ['cyprus problem', 'cyprus issue', 'un envoy', 'negotiations', 'tatar', 'christodoulides', 'bi-zonal', 'reunification', 'holguin', 'kypriako', 'peace talks', 'cyprus talks'];
    return keywords.some(kw => text.includes(kw));
  };

  let filteredStories = data.stories.filter(story => {
  ...
  filteredStories.sort((a, b) => {
    if (sortBy === 'Sources') {
      const aLen = a.articles ? a.articles.length : 0;
      const bLen = b.articles ? b.articles.length : 0;
      return bLen - aLen;
    } else {
      return getStoryDate(b) - getStoryDate(a);
    }
  });

  const cypriotIssueStories = filteredStories.filter(isCypriotIssue);
  const otherStories = filteredStories.filter(s => !isCypriotIssue(s));
    ...
    <div className="container">
      ...
      <div className="controls-bar glass" style={{ padding: '1rem 1.5rem', borderRadius: '12px', display: 'flex', gap: '1rem', marginBottom: '2rem', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        >
          ...
          <option value="Sources">Sort by: Source Count</option>
        </select>
      </div>

      <main className="columns-layout">
        <div className="news-column">
          <h2 className="column-header">Other News</h2>
          <AnimatePresence>
            {otherStories.length === 0 ? (
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="no-stories">No stories match your filters.</motion.p>
            ) : (
              otherStories.map((story, idx) => (
                <StoryCard key={story.headline + idx} story={story} idx={idx} />
              ))
            )}
          </AnimatePresence>
        </div>
        <div className="news-column issue-column">
          <h2 className="column-header">The Cypriot Issue</h2>
          <AnimatePresence>
            {cypriotIssueStories.length === 0 ? (
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="no-stories">No stories match your filters.</motion.p>
            ) : (
              cypriotIssueStories.map((story, idx) => (
                <StoryCard key={story.headline + idx} story={story} idx={idx} />
              ))
            )}
          </AnimatePresence>
